  ...
  confirm() {
    if (this.card == null) {
      if (!this.cardRequest.number || this.cardRequest.number.length < 14) {
        this.translate
          .get("err_field_card_nummber")
          .subscribe((value) => this.global.showToast(value));
      } else if (
        !this.cardRequest.exp_month ||
        this.cardRequest.exp_month.length != 2
      ) {
        this.translate
          .get("err_field_card_exp_month")
          .subscribe((value) => this.global.showToast(value));
      } else if (
        !this.cardRequest.exp_year ||
        this.cardRequest.exp_year.length != 2
      ) {
        this.translate
          .get("err_field_card_exp_year")
          .subscribe((value) => this.global.showToast(value));
      } else if (!this.cardRequest.cvc || !this.cardRequest.cvc.length) {
        this.translate
          .get("err_field_card_cvc")
          .subscribe((value) => this.global.showToast(value));
      } else if (!this.cardRequest.name || !this.cardRequest.name.length) {
        this.translate
          .get("err_field_card_name")
          .subscribe((value) => this.global.showToast(value));
      } else if (!this.cardRequest.email || !this.cardRequest.email.length) {
        this.translate
          .get("err_field_card_email")
          .subscribe((value) => this.global.showToast(value));
      } else {
        this.translate
          .get(["saving", "something_went_wrong"])
          .subscribe((values) => {
            this.global.presentLoading(values["saving"]);

            this.dataService.resources.cards.post(this.cardRequest).subscribe(
              (res) => {
                this.global.dismissLoading();
