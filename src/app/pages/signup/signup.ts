...
export class SignupPage {
  ...
  constructor(
    ...
    private firebaseService: FirebaseClient,
  ...
  async requestSignUp() {
    ...
    if (!this.signUpRequest.name.length) {
      ...
      this.signUpRequest.email.length <= 5 ||
      !reg.test(this.signUpRequest.email)
    ) {
      ...
      let alert = await this.alertCtrl.create({
        ...
        buttons: [
          ...
          },
          {
            text: text["yes"],
            handler: () => {
              this.signUpRequest.mobile_number = this.countryCode + this.phoneNumber;
              this.signUp();
            },
          },
        ],
  ...
  signUp() {
    this.translate.get('signing_up').subscribe(async value => {
      ...
      await this.authService.register(this.signUpRequest).then(res => {
    ...
    this.file
      ...
      .then((buffer) => {
        ...
        this.firebaseService
          .uploadBlob(new Blob([buffer], { type: mime }))
          .then((url) => {
            this.cue.dismissLoading();
